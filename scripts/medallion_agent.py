...
def tool_generate_semantic_view(conn, model_name, analysis_name=None):
    ...
    Profiles columns from Snowflake, classifies them as dimensions vs metrics,
    and generates sem_*.sql, schema.yml metadata, and CREATE SEMANTIC VIEW DDL.
    """
    import subprocess
    ...
    script_path = PROJECT_ROOT / "scripts" / "generate_semantic_view.py"
    if not script_path.exists():
        return {"error": "generate_semantic_view.py script not found in scripts/"}
    ...
    try:
        ...
        if result.returncode == 0:
            return {
                ...
                "files": [
                    f"models/semantic/sem_{analysis_name}.sql",
        ...
        return {"error": "Semantic view generation timed out after 120 seconds"}
    except Exception as e:
        return {"error": f"Failed to generate semantic view: {e}"}
...
TOOLS = {
    ...
    "generate_semantic_view": {
        ...
        "description": "Generate a Snowflake Semantic View from a mart model. Profiles columns, classifies dimensions vs metrics, and generates sem_*.sql, schema.yml metadata, and CREATE SEMANTIC VIEW DDL.",
        "params": {"model_name": "string — mart model name (e.g. 'fct_orders')", "analysis_name": "string (optional — e.g. 'revenue_analysis', defaults to '<entity>_analysis')"},
    },
    "run_dbt": {
        "fn": tool_run_dbt,
...
- **Semantic**: Snowflake Semantic Views — define dimensions and metrics over marts for Cortex Analyst NL querying
...
- Semantic models: `sem_<analysis_name>` (e.g., `sem_revenue_analysis`)
