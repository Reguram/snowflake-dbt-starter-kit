...
**Extension-based scanner packages** may also be installed in the account. These come from the Snowflake Trust Center extension.

Extension packages and custom scanner packages may also exist. Query `snowflake.trust_center.scanner_packages` for the full list. Extension scanners are not in the first-party static mapping — always query the views to look up their IDs.
