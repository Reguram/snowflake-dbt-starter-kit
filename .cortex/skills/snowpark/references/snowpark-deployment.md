...
After (valid SP handler with local debug block):
```python
from snowflake.snowpark import Session
...
**Preferred:** Use `snow init` to bootstrap, then customize:
```bash
snow init <project_name> --template example_snowpark
```

Then edit the generated `snowflake.yml` and handler files to match the user's requirements.

**Alternative:** If `snow init` is unavailable, manually create files using the templates above.
...
6. Write handler file(s) (`procedures.py` and/or `functions.py`)
