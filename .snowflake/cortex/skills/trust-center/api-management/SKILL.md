...
### Static mapping of first-party scanners

The table below covers built-in scanners from the three first-party packages (Security Essentials, CIS Benchmarks, Threat Intelligence). Accounts may also have **extension-based scanner packages** (e.g., Roles, Secrets & Privileged Access, Sharing, Users, Authentication, Configuration) whose scanner IDs are not listed here. For extension scanners, always query the views to get the correct package ID and scanner ID:
